...
*/
void Actor::Initialize(int mesh, GLuint tex, int hp, int state,
...
float MinDistSq(const Actor& rect, const glm::vec3& point)
{
	float dx = glm::max(rect.colLocal.origin.x - point.x,0.0f);
	dx = glm::max(dx, point.x - rect.colLocal.size.x);
	float dy = glm::max(rect.colLocal.origin.y - point.y, 0.0f);
	dy = glm::max(dy, point.y - rect.colLocal.size.y);
	float dz = glm::max(rect.colLocal.origin.z - point.z, 0.0f);
	dz = glm::max(dz, point.z - rect.colLocal.size.z);
...
void ActorsCollision(Actor* va, Actor* vb, CollisionHandlerType func)
{
